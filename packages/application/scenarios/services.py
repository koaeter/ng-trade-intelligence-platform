...
def evaluate_requirement(scenario: ExportScenario, requirement: Requirement, evidence: tuple[Evidence, ...] = ()) -> ApplicabilityEvaluation:
    ...
    return ApplicabilityEvaluation(scenario.id, requirement.id, result, RULE_SET_VERSION, evidence)

def evaluate_scenario(scenario_repository: ExportScenarioRepository, requirement_repository: RequirementRepository, evaluation_repository: ApplicabilityEvaluationRepository, scenario_id: str, evidence_repository: EvidenceRepository | None = None) -> list[ApplicabilityEvaluation]:
    scenario = scenario_repository.get(scenario_id)
    if scenario is None: raise ValueError(f"Export scenario not found: {scenario_id}")
    evaluations = []
    for requirement in requirement_repository.list_all():
        evidence = tuple(item for evidence_id in requirement.evidence_ids if evidence_repository and (item := evidence_repository.get(evidence_id)) is not None)
        evaluations.append(evaluate_requirement(scenario, requirement, evidence))
    for evaluation in evaluations: evaluation_repository.add(evaluation)
    return evaluations
